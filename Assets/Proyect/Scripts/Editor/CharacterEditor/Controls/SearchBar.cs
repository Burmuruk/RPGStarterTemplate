...
namespace Burmuruk.Tesis.Editor.Controls
{
    public class SearchBar : IUIListContainer<BaseCreationInfo>
    {
        ...
        private bool TryEnableCharacterElements(List<(ElementType type, List<string> ids)> elements)
        {
            ...
            nextTurn:
                if (Creations[i].pinned)
                {
                    ...
                    for (int k = 0; k < elements[0].ids.Count; k++)
                    {
                        if (Creations[i].Id == elements[0].ids[k])
                        {
                            enabled = true;
                            RemoveElement(k);
                            ++i;
                ...
                if (elements.Count > 0)
                {
                    Creations[i].SetInfo(false, elements[0].type, elements[0].ids[0], GetName(0));
                    EnableContainer(Creations[i].element, true);
                    ...
                    if (ShowElementColour)
                    {
                        if (ShowCustomColour)
                        {
                            ...
                            Highlight(Creations[i].element, true, GetElementColour(elements[0].type));
                    }
            ...
            string GetName(int idx) =>
                SavingSystem.Data.creations[elements[0].type][elements[0].ids[idx]].Name;

            void RemoveElement(int idx)
            {
                elements[0].ids.RemoveAt(idx);

                if (elements[0].ids.Count <= 0)
                {
                    elements.RemoveAt(0);
                }
            }
        ...
        private void Displace_Pins(int elementIdx)
        {
            ...
            for (int i = elementIdx + 1; i < Count; i++)
            {
